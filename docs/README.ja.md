...
| `p` | プリセットを適用 | MCP サーバを探す | — |
| `g` | MCP サーバを探す | MCP サーバを探す | MCP サーバを探す |
| `y` | — | — | 別のエージェントへコピー |

**`p` と `g` は同じパネルを開きます。** サーバの出どころ二つを一度に検索します。
まず組み込みの九つのレシピを星付きの推奨として、その後に公式レジストリの結果を
並べます。入力は表示済みの一覧を絞り込むだけで、レジストリに問い合わせるのは
`Ctrl+R` だけ — ネットワークに出る唯一のキーです。
